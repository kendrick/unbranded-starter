...
Dispatch that audit; do not ask the user whether to. The weight is the checkpoint Phase 0 gives them, stated in one line and cheap to correct, and the audit is not a second one. The `constitution` skill carries the lone exception, a clause count over its weight's ceiling, which is a decision about the job rather than about the document. The same goes for Phase 1's DEC-audit, which has no exception at all. This is worth stating because the gate got more expensive: since #110 a PASS binds to the constitution's bytes, so a revision after the verdict costs a fresh round, and since #119 that round builds a reference implementation before it can run anything. Both make holding for a review look prudent. Send it anyway. An unreviewed constitution is what the auditor is for, and a round it spends is cheaper than a phase that stalls waiting on a human who was never asked a real question.
...
That PASS also covers a class of defect rather than every class. What a CON-auditor can settle is what a clause carries on its own: a concrete check, text and check that agree about the same artifact, a failing example you can describe, a manifest that parses. What it cannot settle before tasks exist is anything whose falsifiability is a relationship to a schedule—a note delegating coverage to clauses that turn out to run too early, a pair that only contradicts once someone builds the check, a claim turning on task order. Phase 1 owns those. On the #100 run a correct CON-audit PASS was followed by three constitution defects, and the audit that caught them was not chartered to look.

And a PASS bounds what that round reached, not what the document contains. #143 measured it: eight blind DEC rounds against one reconstructed constitution, each handed the charter verbatim with three known defects planted. Every round filed four to six real findings, no charter caught all three every time, and the miss rotated between rounds rather than shrinking. Four charter revisions went in across those rounds, and each worked on the pass it targeted while something else fell out. A round files roughly a fixed number of findings whatever its charter says, so prose steers which defects it reaches, not how many. So some clause defects are decided by luck, and one clean round is not evidence of a clean document—which matters, because a PASS is what opens `dispatch-guard` to every worker. Writing a better charter does not fix that. Taking a class out of the competition does: R22 now proves the delegating-note class against the schedule, so no round spends attention on it again.
...
**Phase 1, decompose.** Invoke the `decompose` skill to turn the spec plus constitution into task files under `.agent-guild/state/tasks/`, each with an executor, a checker, and a `check_method` that cites constitution clauses. Then dispatch the auditor with `Audit-ID: DEC-audit`, which has three jobs: confirm the decomposition covers the spec, re-read the constitution now that there is a schedule to read it against, and run every clause whose check is a command rather than a rubric. Until a DEC-audit PASS verdict exists, `dispatch-guard` blocks every worker, and the latest round is the one that counts. There is no content fingerprint here, only in Phase 0: task files change status all job long, so a digest over them would go stale on the first transition. Coverage is the one property nothing downstream can recover: a checker verifies the task it was given, so a spec section no task covers is never built and every verdict in the job stays green while it happens.

The second job is the one Phase 0 could not do. A clause defect that surfaces here comes back as a constitution defect by clause id, and the fix is a clause revision and another CON round rather than a re-cut task. So expect a DEC FAIL to send you backward into Phase 0 sometimes. That is the mechanism working rather than a decomposition you got wrong, and the alternative is a worker building against the defect.
...
- `.agent-guild/state/apparatus/<Audit-ID>-r<N>/`—scratch an auditor builds in when a clause's check has to be run rather than read: a reference implementation, a probe suite, whatever the checks read. Either audit builds here, which is what the path's `<Audit-ID>` key is for: on a Claude host both run every clause whose check is a command and read the ones whose check is a rubric, while a Codex auditor reads all of them and names what it could not run. A venue an auditor acts *on*—a throwaway repo, a whole-tree copy—goes in its own `mktemp -d` instead, and so does any reference implementation the project's own tooling would read as its own, a template it renders or a file its tests collect. Gitignored is not isolated, because a project's tooling walks its tree without consulting git (`kendrick/dotfiles#22` lost a whole repo's template rendering to one fixture left here). Round-scoped, and that is a finding source rather than housekeeping: a rebuild has to settle whatever axis the clause left unpinned, which is how an ambiguous clause surfaces—on `kendrick/dotfiles#19` that happened twice in consecutive rounds, one graded major and one a blocker. Gitignored and outside the job's diff scope, so none of it ships. A round's directory outlives its round as a later round's comparand—the auditor builds its own first, then diffs against the most recent predecessor whose `SOURCE.sha256` matches what its own build read, and files material divergences against the clause (#198)—so every round's stays in place while the job runs. Teardown deletes them rather than moving them into `archive/`, which is tracked.
...
- `.agent-guild/state/PAUSED`—if this file exists, every hook stands down. Only the user creates it, to hand control back or work around a broken gate. It is the blunt instrument: for the one gate that misfires most, `check-job-spec.py`'s four inferring rules, the constitution's `**Lint exception**:` line silences that rule alone and leaves every other gate standing (#139).
