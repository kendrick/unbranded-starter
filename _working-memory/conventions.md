...
- **Manifest is data.** Behavior differences between units are expressed as declarative fields (`implies`, `requires`, `excludes`, `files`, `postInstall`, `packageJsonPatch`, `removeNotes`), not branching code. Prefer adding a field over special-casing a unit in the runtime.
- **Day-2 verbs split a pure planner from a prompting shell.** `auditRepo` (doctor), `planRemoval`, and `planUpdate` are filesystem-in/plan-out — they read disk, state, and templates and return a plan without writing anything; only the `run*` shells prompt, write files, and touch state. State mutation itself goes through the three doors in `src/state/state.ts` (`writeStateFile` grows, `applyRemovalToState` shrinks, `refreshTrackedFiles` refreshes). Build any new verb on this split.
